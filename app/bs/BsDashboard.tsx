...
export default function BsDashboard({ bso, apm, period, uploadedAt }: {
...
function PagePodium({ people, perfect, allPerfect }: { people: BPerson[]; perfect: BPerson[]; allPerfect: boolean }) {
  if (allPerfect || perfect.length > 3) return <BestPerfectList names={perfect.map(p => ({ name: p.name, sub: [p.regional, p.apm].filter(Boolean).join(' · ') }))} />;
...
function PodiumCard({ rank, p }: { rank: 1 | 2 | 3; p: BPerson }) {
  ...
  return (
    <div className={`relative rounded-2xl border p-5 md:p-7 flex flex-col items-center text-center gap-2 ${s.height}`} style={{ background: s.cardBg, boxShadow: s.glow, borderColor: s.border }}>
      ...
      <div className={`${big ? 'w-28 h-28 text-4xl' : 'w-20 h-20 text-2xl'} rounded-full flex items-center justify-center font-black text-white shadow`} style={{ background: s.avatarBg }}>{initials(p.name)}</div>
