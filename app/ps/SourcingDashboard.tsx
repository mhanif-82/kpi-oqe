...
export default function SourcingDashboard({ peopleSearch, centralSourcing, period, uploadedAt }: {
  ...
  const worstPS = useMemo(() => [...peopleSearch].sort((a, b) => a.total - b.total).slice(0, 5), [peopleSearch]);
  ...
  return (
    <div className="h-screen bg-[#0a0a0a] text-zinc-100 p-6 md:p-8 flex flex-col gap-6 overflow-hidden">
      ...
      <div key={animKey} className={dir === 'right' ? 'slide-right' : 'slide-left'} style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        {page === 0 && <PagePodium people={bestPS} />}
        {page === 1 && <PageWorst people={worstPS} totalCount={peopleSearch.length} />}
        {page === 2 && <PageCSM ranked={csmRanked} />}
        {page === 3 && <PageLeaderboard ranked={psRanked} scrollSpeed={scrollSpeed} onScrollDone={onLeaderboardDone} />}
      </div>
    </div>
  );
}
...
function PagePodium({ people }: { people: SPerson[] }) {
...
/* ─── Worst People Search ─── */
function PageWorst({ people, totalCount }: { people: SPerson[]; totalCount: number }) {
  return (
    <div className="flex-1 min-h-0 flex flex-col">
      <div className="flex items-baseline gap-3 mb-4 shrink-0">
        <h2 className="text-2xl md:text-3xl font-bold flex items-center gap-2 text-red-400">⚠ Top 5 Terbawah People Search <span className="animate-pulse">(warning!!)</span></h2>
      </div>
      <div className="flex-1 min-h-0 grid gap-3" style={{ gridTemplateRows: `repeat(${Math.max(people.length, 1)}, minmax(0, 1fr))` }}>
        {people.map((p, i) => {
          ...
          return (
            <div key={p.name} className="bg-zinc-900/60 border border-zinc-800 rounded-xl overflow-hidden flex items-stretch h-full">
              ...
              <div className="flex-1 grid grid-cols-12 gap-3 items-center px-5">
                <div className="col-span-1 text-2xl md:text-3xl font-black text-red-400">#{totalCount - i}</div>
...
function PageLeaderboard({ ranked, scrollSpeed, onScrollDone }: { ranked: SPerson[]; scrollSpeed: number; onScrollDone: () => void }) {
  ...
  return (
    <section className="flex-1 flex flex-col min-h-0">
      <h2 className="text-2xl md:text-3xl font-bold mb-4 flex items-center gap-2">📊 Leaderboard People Search <span className="text-base text-zinc-500 font-normal">— {total} orang</span></h2>
